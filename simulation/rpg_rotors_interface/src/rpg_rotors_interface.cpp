...
void RPGRotorsInterface::lowLevelControlLoop(const ros::TimerEvent& time)
{
  ...
  else
  {
        ...
        == control_command_.ANGULAR_ACCELERATIONS)
    {
      ...
      torques_and_thrust.body_torques = inertia_
          * quadrotor_common::geometryToEigen(
              control_command_.angular_accelerations)
          + quadrotor_common::geometryToEigen(control_command_.bodyrates).cross(
              inertia_
                  * quadrotor_common::geometryToEigen(
    ...
    else if (control_command_.control_mode == control_command_.ROTOR_THRUSTS)
    {
      if (control_command_.rotor_thrusts.size() != 4)
      {
        ROS_ERROR_THROTTLE(
            1, "[%s] Require exactly 4 rotor thrusts in case of ROTOR_THRUSTS "
            "control mode.",
      ...
      for (int i = 0; i < 4; i++)
      {
        double motor_speed_squared = control_command_.rotor_thrusts[i]
            / rotor_thrust_coeff_;
...
{
  Eigen::Quaterniond q_e = attitude_estimate.inverse()
      * attitude_cmd.orientation;
...
// pitch and roll control
  if (q_e.w() >= 0)
  {
    body_rate_command.bodyrates.x() = attitude_cmd.bodyrates.x()
        + 2.0 * roll_pitch_cont_gain_ * q_e.x();
    body_rate_command.bodyrates.y() = attitude_cmd.bodyrates.y()
    ...
    const Eigen::Vector3d& body_rate_estimate)
{
  ...
  control_error.segment(3, 3) = rate_cmd.bodyrates.cross(
      inertia_ * rate_cmd.bodyrates) + inertia_ * rate_cmd.angular_accelerations
    ...
    const TorquesAndThrust& torques_and_thrust)
{
  ...
  rotor_speed_cmds.angular_velocities[0] = ((arm_length_
      * torques_and_thrust.body_torques.z()
      ...
      / (4.0 * rotor_drag_coeff_ * arm_length_)) / rotor_thrust_coeff_;
  rotor_speed_cmds.angular_velocities[1] = ((2.0 * rotor_drag_coeff_
      * torques_and_thrust.body_torques.x()
      - arm_length_ * torques_and_thrust.body_torques.z()
      + rotor_drag_coeff_ * arm_length_ * mass_
          * torques_and_thrust.collective_thrust)
      / (4.0 * rotor_drag_coeff_ * arm_length_)) / rotor_thrust_coeff_;
  rotor_speed_cmds.angular_velocities[2] = ((2.0 * rotor_drag_coeff_
      * torques_and_thrust.body_torques.y()
      + arm_length_ * torques_and_thrust.body_torques.z()
      + rotor_drag_coeff_ * arm_length_ * mass_
          * torques_and_thrust.collective_thrust)
      / (4.0 * rotor_drag_coeff_ * arm_length_)) / rotor_thrust_coeff_;
  rotor_speed_cmds.angular_velocities[3] = (-(2.0 * rotor_drag_coeff_
      * torques_and_thrust.body_torques.x()
      + arm_length_ * torques_and_thrust.body_torques.z()
      - rotor_drag_coeff_ * arm_length_ * mass_
          * torques_and_thrust.collective_thrust)
  ...
  for (int i = 0; i < 4; i++)
  {
    quadrotor_common::limit(&rotor_speed_cmds.angular_velocities[i], 0.0,
                            pow(max_rotor_speed_, 2.0));
    rotor_speed_cmds.angular_velocities[i] = sqrt(
